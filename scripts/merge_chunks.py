"""Merge N chunk dirs into a single aggregate dir with unified results.csv +
summary.json matching the layout plot_pact_vs_baseline.py / significance_pact_vs_baseline.py expect.

Chunks are concatenated in the order they are passed; run_NN/ subdirs are
renumbered sequentially across chunks.
"""
...
def main() -> int:
    ...
    p.add_argument("--chunk", action="append", required=True,
                   help="One chunk dir; pass multiple times for multi-chunk merge.")
    p.add_argument("--out", required=True)
    p.add_argument("--copy", action="store_true",
                   help="Copy run subdirs into out; otherwise symlink (faster).")
    args = p.parse_args()
    ...
    for chunk in args.chunk:
        ...
        runs = _gather_runs(chunk_dir)
        print(f"[merge] {chunk_dir.name}: {len(runs)} runs")
