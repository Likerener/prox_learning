"""Launch N parallel datagen subprocesses for FrankaSkinPickAndPlaceOneHouseMugConfig.

Each subprocess gets its own `output_dir` and `seed` so the workers don't
collide on `house_1/episode_*.mp4` / `trajectories_batch_1_of_1.h5` (the
upstream `setup_house_dirs` keys on `house_id` only — see config docstring).
...
Usage:
  ...
  python scripts/run_v3_parallel.py --jobs 4 --samples_per_job 63

  # Or for a small smoke (1 job × 2 ep, ~13 min):
  python scripts/run_v3_parallel.py --jobs 1 --samples_per_job 2 --tag smoke
"""
...
DEFAULT_OUTPUT_BASE = REPO_ROOT / "assets" / "datagen" / "pick_and_place_one_house_mug_v3"
...
def main() -> int:
    ...
    for i in range(args.jobs):
        ...
        # Stagger child start so the heavy scene-load phase doesn't all hit
        # disk/GPU at the exact same instant.
        time.sleep(1.5)
